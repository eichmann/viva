...
<table>
	<tr>
		<td>
			<c:if test="${limit > 0 && offset > 0}"><a href="Special_groupList.jsp?limit=${limit}&offset=${offset - limit}">&lt;</a></c:if>
		</td>
		<td>
			<c:if test="${limit > 0}"><a href="Special_groupList.jsp?limit=${limit}&offset=${offset + limit}">&gt;</a></c:if>
		</td>
	</tr>
</table>

<ul>
<viva:foreachSpecial_group limitCriteria="${limit}" offsetCriteria="${offset}" >
   <viva:Special_group>
